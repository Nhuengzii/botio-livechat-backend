...
func (c *config) handler(ctx context.Context, request events.APIGatewayProxyRequest) (_ events.APIGatewayProxyResponse, err error) {
	...
	if !ok { // no need to query with filter
		stdConversations, err = c.dbClient.QueryConversations(ctx, shopID, pageID, offsetPtr, limitPtr)
		if err != nil {
			return apigateway.NewProxyResponse(500, "Internal Server Error", "*"), err
		}
		...
		if filter.Message != "" && filter.ParticipantsUsername != "" {
			...
			stdConversations, err = c.dbClient.QueryConversationsWithParticipantsName(ctx, shopID, stdconversation.PlatformInstagram, pageID, filter.ParticipantsUsername, offsetPtr, limitPtr)
			if err != nil {
				return apigateway.NewProxyResponse(500, "Internal Server Error", "*"), err
			}
		} else if filter.Message != "" { // query with message
			stdConversations, err = c.dbClient.QueryConversationsWithMessage(ctx, shopID, stdconversation.PlatformInstagram, pageID, filter.Message, offsetPtr, limitPtr)
			if err != nil {
				return apigateway.NewProxyResponse(500, "Internal Server Error", "*"), err
			}
		}
