...
func (c *config) handler(ctx context.Context, request events.APIGatewayProxyRequest) (_ events.APIGatewayProxyResponse, err error) {
	...
	if !ok { // no need to query with filter
		stdConversations, err = c.dbClient.ListConversations(ctx, shopID, stdconversation.PlatformInstagram, pageID, skipPtr, limitPtr)
		if err != nil {
		...
		if filter.Message != "" && filter.ParticipantsUsername != "" {
			...
			stdConversations, err = c.dbClient.ListConversationsWithParticipantsName(ctx, shopID, stdconversation.PlatformInstagram, pageID, filter.ParticipantsUsername, skipPtr, limitPtr)
			if err != nil {
				return apigateway.NewProxyResponse(500, "Internal Server Error", "*"), err
			}
		} else if filter.Message != "" { // query with message
			stdConversations, err = c.dbClient.ListConversationsWithMessage(ctx, shopID, stdconversation.PlatformInstagram, pageID, filter.Message, skipPtr, limitPtr)
			if err != nil {
				return apigateway.NewProxyResponse(500, "Internal Server Error", "*"), err
			}
		}
